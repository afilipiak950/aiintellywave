...
interface JobOffersToggleProps {
  isEnabled: boolean;
  onToggle: () => Promise<void>;
  isLoading: boolean;
}

export const JobOffersToggle = ({ 
  isEnabled, 
  onToggle, 
  isLoading 
}: JobOffersToggleProps) => {
  const handleToggle = async () => {
    console.log(`[Feature Debug] Toggling Job Offers feature from ${isEnabled} to ${!isEnabled}`);
    await onToggle();
  };
    ...
    <div className="space-y-2">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <span className="text-sm">Job Offers Feature (Jobangebote)</span>
          {isEnabled && (
